...
- [화살표 함수에 대해 설명해주세요](#화살표-함수에-대해-설명해주세요)
- [mutable한 값과 immutable한 값의 저장 방식의 차이에 대해서 설명해줄 수 있나요?](#mutable한-값과-immutable한-값의-저장-방식의-차이에-대해서-설명해줄-수-있나요)
- [제너레이터 문법에 대해 설명해주세요](#제너레이터-문법에-대해-설명해주세요)
...
화살표 함수는 function 키워드 대신 화살표를 이용해 보다 간략한 방법으로 함수를 선언할 수 있습니다.

자바스크립트의 화살표 함수는 기존 함수와는 다르게 실행 컨텍스트 생성시 this가 정적으로 결정됩니다.
이를 Lexical this라고 표현하며, 화살표 함수 내에서 선언되는 this는 무조건 자신이 선언되는 컨텍스트에서 가장 가까운 상위 컨텍스트의 this를 가리킵니다.

이는 기존 자바스크립트가 메서드가 아닌 함수 내부에서 선언된 this가 전역 객체를 가리키는 설계상 오류를 해결할 수 있는 방법입니다.

이런 화살표 함수의 특징상, 객체의 메서드로 화살표 함수를 이용하는 것은 추천하지 않는 방식입니다.
일반 함수는 메서드를 호출헌 객체를 this에 저장하지만, 화살표 함수는 객체의 상위 컨텍스트를 바라보기 때문에 오히려 오해를 불러 일으킬 수 있습니다.

## mutable한 값과 immutable한 값의 저장 방식의 차이에 대해서 설명해줄 수 있나요?

[객체와 변경불가성](https://poiemaweb.com/js-immutability)

불변하는 기본형 타입의 경우 변수, 데이터 영역을 활용하여 메모리 내 데이터를 저장할 때 변수 영역 내 값을 변경하기 위해선
새로 데이터 영역에 값을 할당하는 과정이 필요합니다. 따라서 한번 생성한 기본형 타입은 불변할 수 밖에 없으며, 변수 영역은 데이터 영역의 주솟값을 가지게 됩니다.

가변적인 참조형 타입의 경우 변수, 데이터, 객체 영역을 사용합니다. 객체 영역 내에서 참조하는 데이터 영역의 값이 변경되더라도
데이터 영역의 값이 변한것일 뿐, 객체 영역의 메모리 주소는 동일하므로 가변성을 띄게 됩니다. 이 과정에서 객체 변수는 데이터 영역의 주솟값을 복사하고,
데이터 영역은 객체 영역의 주솟값을 복사합니다.

즉 기본형은 데이터를 할당하기 위한 주소값 복사가 한번만 이루어 지지만, 참조형은 한단계를 더 거치게 되는 차이가 있습니다.

## 제너레이터 문법에 대해 설명해주세요

일반 함수는 보통 return 키워드를 이용해 하나의 값만을 반환하지만, 제너레이터 함수는 필요에 따라 값을 하나씩 반환할 수 있습니다.

제너레이터 함수는 function\* 이라는 키워드를 이용해 선언할 수 있습니다.
제너레이터 함수를 호출하면 함수가 실행되지 않고, 실행을 처리하는 객체인 제너레이터 객체를 반환합니다.

제너레이터 객체는 next()라는 메서드를 이용해 다음 yield 키워드의 값을 반환합니다.
yield 키워드와 next 메서드를 이용해 함수 호출자에게 제어권을 양도하고 필요한 시점에 이어서 실행할 수 있습니다.
