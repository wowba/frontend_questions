...
- [세그먼트 트리에 대해 설명해주세요](#세그먼트-트리에-대해-설명해주세요)
...
세그먼트 트리는 특정 구간 내 연산을 빠르게 구할 수 있는 트리입니다.

세그먼트 트리는 이진 트리에 기반하며, 각 배열의 합을 저장하여 각 노드에서는 아래 노드의 합을저장하며, 루트노드에서는 모든 배열 요소의 합을 저장합니다.

세그먼트 트리를 생성한 이후 배열 내 요소가 업데이트 된다면, 해당 배열과 관계된 노드들이 업데이트 되어 모든 요소에 대해 계산을 수행하지 않아도 최소한의 횟수로
각 노드의 데이터를 최신화 해 특정 구간의 연산을 빠르게 구할 수 있습니다.
